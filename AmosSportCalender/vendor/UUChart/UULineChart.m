...
-(void)setXLabels:(NSArray *)xLabels
{
    ...
    for (int i=0; i<xLabels.count; i++) {
        ...
        UUChartLabel * label = [[UUChartLabel alloc] initWithFrame:CGRectMake(i * _xLabelWidth+UUYLabelwidth, self.frame.size.height - UULabelHeight - 1, _xLabelWidth, UULabelHeight)];
...
//        view.backgroundColor = [_colors objectAtIndex:index]?[_colors objectAtIndex:index]:UUGreen;
        ...
        label.text = [NSString stringWithFormat:@"%d天",(int)value];
        [self addSubview:label];
    }
