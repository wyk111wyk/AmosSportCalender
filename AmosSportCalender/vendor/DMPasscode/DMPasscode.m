...
- (void)showPasscodeInViewController:(UIViewController *)viewController completion:(PasscodeCompletionBlock)completion {
    NSAssert([self isPasscodeSet], @"没有设置密码");
    _completion = completion;
...
- (void)openPasscodeWithMode:(int)mode viewController:(UIViewController *)viewController {
    ...
    if (_mode == 0) {
        [_passcodeViewController setInstructions:@"请输入新的密码"];
    } else if (_mode == 1) {
        [_passcodeViewController setInstructions:@"请输入密码以解锁"];
    }
}
...
- (void)enteredCode:(NSString *)code {
    if (_mode == 0) {
        if (_count == 0) {
            _prevCode = code;
            [_passcodeViewController setInstructions:@"请重复输入"];
            ...
            if ([code isEqualToString:_prevCode]) {
                ...
                [_passcodeViewController setInstructions:@"请输入新的密码"];
                [_passcodeViewController setErrorMessage:@"你的密码并不符合\n请重新尝试"];
                [_passcodeViewController reset];
                _count = 0;
                return;
            }
        ...
        if ([code isEqualToString:[[DMKeychain defaultKeychain] objectForKey:KEYCHAIN_NAME]]) {
            ...
            if (_count == 1) {
                [_passcodeViewController setErrorMessage:@"剩余1次尝试机会"];
            } else {
                [_passcodeViewController setErrorMessage:[NSString stringWithFormat:@"剩余%i次尝试机会", 2 - _count]];
            }
            [_passcodeViewController reset];
